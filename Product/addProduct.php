	...
	if (array_key_exists('Authorization', $headers) && preg_match('/Bearer\s(\S+)/', $headers['Authorization'], $matches)){
		...
		if(verifyToken($matches[1])){
            ...
            if($checkDuplicateProduct['total'] == 0){
                ...
                mysqli_query($conn, "INSERT INTO `product_variant`(`VariantTitle`, `ProductID`, `UnitID`, `MRP`, `Price`, `AvailableQuantity`, `isActive`, `Modified_At`)  VALUES ('$VariantTitle','$ProductID','$UnitID','$MRP','$Price',NULL,'1','$CurrendDateTime')");
